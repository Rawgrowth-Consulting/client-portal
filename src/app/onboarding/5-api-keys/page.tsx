...
export default function ApiKeysStep() {
  ...
  const submitKey = useMutation(api.apiIntegrations.submit);
  ...
  // Load already-submitted keys
  const existingKeys = useQuery(
    api.apiIntegrations.list,
    clientId ? { clientId: clientId as Id<'clients'> } : 'skip'
  );

  useEffect(() => {
    if (existingKeys) {
      const alreadySubmitted = new Set(existingKeys.map((k) => k.platform));
  ...
  async function handleContinue() {
    if (!clientId) return;
    await updateStep({ clientId: clientId as Id<'clients'>, step: 6 });
