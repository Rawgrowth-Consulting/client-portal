...
export async function computeOnboardingProgress(
  ...
  const { data: client } = await supabaseAdmin
    .from("clients")
    .select("messaging_channel, onboarding_step")
    .eq("id", userId)
    .maybeSingle();

  if (client?.messaging_channel || (client?.onboarding_step ?? 1) >= 2) {
  ...
  if ((client?.onboarding_step ?? 1) >= 5) completed.push("brandDocs");

  // Section 6 — software access complete (step >= 6)
  if ((client?.onboarding_step ?? 1) >= 6) completed.push("softwareAccess");

  // Section 7 — scheduled calls complete (step >= 7)
  if ((client?.onboarding_step ?? 1) >= 7) completed.push("scheduleCalls");

  // Section 8 — onboarding fully completed (client.status flips to 'active')
  const { data: clientDone } = await supabaseAdmin
    .from("clients")
    .select("status")
    .eq("id", userId)
    .maybeSingle();
  if (clientDone?.status === "active") completed.push("onboardingComplete");
